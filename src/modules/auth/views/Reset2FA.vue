  ...
  <div class="bc-verify">
    ...
    <div class="content" v-if="isLoading">
      <div class="be-flex jc-space-center verify-code">
        <base-icon v-if="isSuccess" icon="icon-reset2fa-succsess" size="80" />
        <base-icon v-else icon="icon-reset2fa-failed" size="80" />
      </div>
      <div class="text-3xl text-bold text-center mb-24 title">
        <span v-if="isSuccess">{{ $t('verify.title-reset2fa-succsess') }}</span>
        <span v-else>{{ $t('verify.title-reset2fa-failed') }}</span>
      </div>
      <div class="text-base text-center mb-24 des">
        <span v-if="isSuccess">{{ $t('verify.des-reset2fa-succsess') }}</span>
        <span v-else>{{ $t('verify.des-reset2fa-failed') }} {{ urlSystem['system.token.support_email'] }}</span>
      </div>
...
<script lang="ts">
  ...
  export default class Reset2FA extends Vue {
